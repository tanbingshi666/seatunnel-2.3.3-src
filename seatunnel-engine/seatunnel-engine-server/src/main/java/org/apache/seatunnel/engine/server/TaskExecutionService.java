...
public class TaskExecutionService implements DynamicMetricsProvider {
    ...
    public void start() {
        // 1 基于 newCachedThreadPool 模式创建线程池 (线程池最大值为 Integer.MAX_VALUE)
        // 2 基于 LinkedBlockingDeque 创建阻塞队列缓存 TaskTracker
        // 3 基于 RunBusWorkSupplier 创建 CooperativeTaskWorker 线程
        // 4 CooperativeTaskWorker 持续从 2 阻塞队列获取 TaskTracker 执行
    ...
    public PassiveCompletableFuture<TaskExecutionState> deployLocalTask(
        ...
        try {
            ...
            // 5 提交线程共享任务
            // 最终调用 CooperativeTaskWorker.run()
