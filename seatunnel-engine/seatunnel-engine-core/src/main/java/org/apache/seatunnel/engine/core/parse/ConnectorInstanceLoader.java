...
public class ConnectorInstanceLoader {
    ...
    public static ImmutablePair<SeaTunnelSource, Set<URL>> loadSourceInstance(
        ...
        // 入口 如果是本地 IDEA 启动 返回空
        List<URL> pluginJarPaths = sourcePluginDiscovery.getPluginJarPaths(Lists.newArrayList(pluginIdentifier));

        // 根据 pluginName (也即 result_table_name) 查询 SeaTunnelSource 子类
        // 优先从 classpath 加载 其次从 ${SEATUNNEL_HOME}/connectors/seatunnel 目录下的 JAR 加载
        // 比如 FakeSource -> FakeSource
