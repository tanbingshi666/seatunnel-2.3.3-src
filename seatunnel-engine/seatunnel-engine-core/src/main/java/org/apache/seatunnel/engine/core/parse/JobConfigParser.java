...
public class JobConfigParser {
    ...
    public Tuple2<CatalogTable, Action> parseSource(
        ...
        // 1 获取 source 对应的 jars URL
        ImmutablePair<SeaTunnelSource, Set<URL>> tuple =
                ConnectorInstanceLoader.loadSourceInstance(config, jobConfig.getJobContext(), commonPluginJars);
        ...
        // 格式: Source[%s]-%s-%s
        String actionName =
                createSourceActionName(0, config.getString(CollectionConstants.PLUGIN_NAME), getTableName(config));

        // 4 创建 SourceAction
        SourceAction action = new SourceAction(
                idGenerator.getNextId(), actionName, tuple.getLeft(), tuple.getRight());
        action.setParallelism(parallelism);

        // 5 获取 source 字段 schema 类型
        SeaTunnelRowType producedType = (SeaTunnelRowType) tuple.getLeft().getProducedType();

        // 6 将 source 封装为 CatalogTable (五部分组成：tableId, tableSchema, options, partitionKeys, comment)
